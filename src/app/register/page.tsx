  ...
  const onSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(AUTH_FUNCTION_URL, {
        ...
        body: JSON.stringify({ email: email.trim(), register: true }),
      });

      if (res.ok) {
        addToast({
          type: "success",
          title: "Registrierungslink versendet",
          description: "Bitte prüfen Sie Ihr Postfach und klicke Sie auf den Link, um Ihre Registrierung abzuschließen.",
          duration: 6000,
        });
        setEmail("");
      } else {
        ...
        addToast({
          ...
          description: details?.slice(0, 300) || "Der Registrierungslink konnte nicht gesendet werden.",
          duration: 6000,
        });
      }
    } catch (error) {
      ...
      <Card className="w-full">
        ...
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <p className="text-center text-xs text-gray-500 mt-2">Bereits ein Konto? <a href="/login" className="underline">Anmelden</a></p>
            <p className="text-center text-xs text-gray-500">
              Wir senden Ihnen einen einmaligen Registrierungs‑Link an Ihre E‑Mail-Adresse.
            </p>
          </form>
        </CardContent>
      </Card>
