...
export default function InvestmentCaseLB33() {
  ...
  const [currentName, setCurrentName] = useState<string | null>(() => {
    try {
      return localStorage.getItem("lb33_current_project");
    } catch {
      return null;
    }
  });
  ...
  const saveProject = () => {
    ...
    if (!name) {
      name = prompt("Projektname eingeben:", "Projekt Titel")?.trim() || "";
      if (!name) return;
    }
    ...
    const newProjects = { ...projects, [name]: data };
  ...
  const saveProjectAs = () => {
    const name = prompt("Projektname eingeben:", "Projekt Titel")?.trim();
    if (!name) return;
    const data: ProjectData = {
      cfgCases,
      finCases,
  ...
  useEffect(() => {
    const name = searchParams.get("name") ?? localStorage.getItem("lb33_current_project");
    if (name) {
      loadProject(name);
    }
        ...
        <div className="space-y-6 h-full overflow-y-auto pr-1">
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" onClick={saveProjectAs}>Speichern unter …</Button>
